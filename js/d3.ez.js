...
d3.ez.circularHeatChart = function module() {
    ...
    var dispatch = d3.dispatch("customHover");
    function init(data) {
        //radialLabels = d3.values(data[0].values).map(function(d) {
        //    return d.key;
        //});
        ...
        //segmentLabels = data.map(function(d) {
        //    return d.key;
        //});
    ...
    function my(selection) {
        selection.each(function(data) {
            ...
            //var color = ['#edf3f8' , '#b6cee2', '#6c9dc6' , '#4682b4', '#325d81']
            //seg=Math.floor(Math.random() * 10) ;
            seg=countryDict["Mauritius"];
            // Arc Generator
            ...
            //alert(d.ring)
              ...
              if( i == seg){
                ...
                return innerRadius + segmentHeight + 5 * segmentHeight +32;
            })
	    		.innerRadius(function() {
                return innerRadius + segmentHeight + 5 * segmentHeight;
            });
            // Rings
            ...
            radLabels.selectAll("def").data(radialLabels).enter().append("def").append("path").attr("id", function(d, i) {
                ...
                var r = innerRadius + (i + .4) * segmentHeight;
	  	...
	  	return "translate(" + labelArc.centroid(d,i)[0] + "," + labelArc.centroid(d,i)[1] + ") rotate(-90) rotate(" + (midAngle * 180/Math.PI) + ")"; })
            .text(function(d) {
                return d;
            });
